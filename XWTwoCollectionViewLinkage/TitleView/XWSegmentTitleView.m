...
@interface XWSegmentTitleView ()<UIScrollViewDelegate>
...
@property (nonatomic, assign) XWIndicatorType indicatorType;

@property (nonatomic, strong) NSArray *titlesArr;

@end

@implementation XWSegmentTitleView
...
- (void)layoutSubviews
{
    ...
    if (_titleFont != _titleSelectFont) {
        for (int idx = 0; idx < self.titlesArr.count; idx++) {
            ...
            CGFloat itemBtnWidth = [XWSegmentTitleView getWidthWithString:self.titlesArr[idx] font:titleFont] + self.itemMargin;
    ...
    else
    {
        for (NSString *title in self.titlesArr) {
            CGFloat itemBtnWidth = [XWSegmentTitleView getWidthWithString:title font:titleFont] + self.itemMargin;
            totalBtnWidth += itemBtnWidth;
        }
    }
...
- (void)moveIndicatorView:(BOOL)animated
{
    ...
    CGFloat indicatorWidth = [XWSegmentTitleView getWidthWithString:self.titlesArr[self.selectIndex] font:titleFont];
    [UIView animateWithDuration:(animated?0.05:0) animations:^{
        switch (_indicatorType) {
            case XWIndicatorTypeDefault:
                ...
                break;
            case XWIndicatorTypeEqualTitle:
...
- (void)btnClick:(UIButton *)btn
{
    ...
    if (self.delegate&&[self.delegate respondsToSelector:@selector(XWSegmentTitleView:startIndex:endIndex:)]) {
        [self.delegate XWSegmentTitleView:self startIndex:self.selectIndex endIndex:index];
    }
...
{
    if (self.delegate&&[self.delegate respondsToSelector:@selector(XWSegmentTitleViewWillBeginDragging:)]) {
        [self.delegate XWSegmentTitleViewWillBeginDragging:self];
    }
}

- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
    if (self.delegate&&[self.delegate respondsToSelector:@selector(XWSegmentTitleViewWillEndDragging:)]) {
        [self.delegate XWSegmentTitleViewWillEndDragging:self];
    }
}
